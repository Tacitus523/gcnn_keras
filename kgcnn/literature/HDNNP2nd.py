...
def make_model_behler(inputs: list = None,
    ...
    Returns:
        :obj:`tf.keras.models.Model`
    """
    assert max(g2_kwargs.get("elemental_mapping")) < mlp_kwargs.get("num_relations"), \
        "Elemental mapping in g2_kwargs exceeds num_relations in mlp_kwargs."
    assert max(g4_kwargs.get("elemental_mapping")) < mlp_kwargs.get("num_relations"), \
        "Elemental mapping in g4_kwargs exceeds num_relations in mlp_kwargs."
...
model_default_inverse_distances = {
    "name": "HDNNP2nd_inverse_distance",
    "inputs": [{"shape": (None,), "name": "node_number", "dtype": "int64", "ragged": True},
               {"shape": (None, 3), "name": "node_coordinates", "dtype": "float32", "ragged": True},
               {"shape": (None, 2), "name": "range_indices", "dtype": "int64", "ragged": True}],
    "mlp_kwargs": {"units": [64, 64, 1],
...
@update_model_kwargs(model_default_inverse_distances)
def make_model_inverse_distances(inputs: list = None,
                      node_pooling_args: dict = None,
                      name: str = None,
                      verbose: int = None,
                      normalize_kwargs: dict = None,
    ...
    r"""Make 2nd generation `HDNNP <https://arxiv.org/abs/1706.08566>`_ graph network via functional API, but use inverse distances as representation.

    Default parameters can be found in :obj:`kgcnn.literature.HDNNP2nd.model_default_inverse_distances`.

    Inputs:
        list: `[node_number, node_coordinates, edge_indices]`
    ...
    # Inverse distance representation.
    layer_pos = NodePosition()
    layer_dist = NodeDistanceEuclidean()
    xi, xj = layer_pos([xyz_input, edge_index_input])
    rij = layer_dist([xi, xj]) # (batch, n*(n-1), 1)
    # rij = tf.map_fn(lambda distances,nodes: tf.reshape(distances, (nodes.shape[0],nodes.shape[0]-1)), [rij, node_input], fn_output_signature=tf.RaggedTensorSpec(shape=[None, None], dtype=rij.dtype))
    #rij = 1/rij
    rij = tf.reshape(rij, (-1,15,14))
    mask = tf.linalg.band_part(tf.ones((15, 14), dtype=tf.bool), 0, -1)  # Only include upper triagonal matrix
    #mask = tf.expand_dims(mask, axis=0)  # Shape: (1, 15, 14)
    #mask = tf.broadcast_to(mask, tf.shape(rij))
    flat_triangle = tf.boolean_mask(rij, mask, axis=1) # Shape: (batch, 15*14/2)
    flat_triangle = tf.reshape(flat_triangle, (-1, int(15*14/2)))
    


    # learnable NN.
    out = MLP(**mlp_kwargs)(flat_triangle)
    #out = PoolingNodes(**node_pooling_args)(n)

    model = ks.models.Model(
        inputs=[node_input, xyz_input, edge_index_input], outputs=out, name=name)
