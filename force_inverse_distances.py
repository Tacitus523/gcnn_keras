...
from kgcnn.literature.HDNNP2nd import make_model_inverse_distances as make_model
...
MODEL_PREFIX = "model_energy_force_inverse_distance" # Will be used to save the models
...
ENERGY_HIDDEN_LAYERS         = [2000, 1600, 1000, 500, 250, 100, 50] # List of number of nodes per hidden layer 
ENERGY_HIDDEN_ACTIVATION     = ["swish"]*7 # List of activation functions of hidden layers
ENERGY_BATCH_SIZE            = 256 # Batch size during training
ENERGY_EARLY_STOPPING        = 0 # Patience of Early Stopping. If <1, no Early Stopping
FORCE_LOSS_FACTOR            = 100
...
# to ensure, that all distances are considered no matter the distance
from itertools import permutations
for data_index, datapoint in enumerate(dataset):
    datapoint["range_indices"] = list(permutations(range(len(datapoint["node_number"])),2))
    assert len(datapoint["range_indices"]) == 210, f"datapoint {data_index} failed assertion"
...
inputs = dataset[:1].tensor(model_config["inputs"])
...
# xi, xj = layer_pos(inputs[1:])
# rij = layer_dist([xi, xj])
# print(rij.shape)
# print(rij)
...
plot_predict_true(predicted_energy, true_energy,
    ...
    model_name=model_name, dataset_name=DATASET_NAME, target_names="Energy",
...
plot_predict_true(predicted_force, true_force,
    ...
    model_name=model_name, dataset_name=DATASET_NAME, target_names="Force",
    error="RMSE", file_name=f"predict_force.png", show_fig=False)

plot_train_test_loss(hists,
    filepath="", data_unit="Eh",
    model_name=model_name, dataset_name=DATASET_NAME, file_name="loss.png", show_fig=False)
